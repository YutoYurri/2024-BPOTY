// List of names
const names = ["Max", "Josh", "Naijal", "Lilly", "Smriti"];

// Wait until the DOM is fully loaded
document.addEventListener("DOMContentLoaded", () => {
  const nameDisplay = document.querySelector(".p6__child"); // Target the nested child
  ...
  if (!nameDisplay) {
    console.error("Element with class '.p6__child' not found.");
    return;
  }
  ...
  const startDelay = 25000; // 25 seconds in milliseconds
  ...
  setTimeout(() => {
    ...
    const randomize = setInterval(() => {
      ...
      if (currentIteration >= iterations) {
        ...
        nameDisplay.textContent = "Smriti"; // Final chosen name
